...
| `reference/train_pairwise_gbm.py`, `gate_pairwise_tiebreak.py` | Modèle de comparaison pairwise Gradient Boosting, branché dans `search.greedy_action(..., tiebreak=...)` pour départager les candidats presque à égalité de gain exact -- **bat B 74/100 au gating**, le bot le plus fort du dépôt à ce jour (mais coûteux : ×140 sur le temps par partie, pas encore utilisable dans un rollout MCTS). Voir `reference/MODELS.md`. |
