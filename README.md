...
| PUCT (`search.py`, `Node.uct_select`/`MCTS(policy_prior=...)`) | Prior de politique appris pour guider la sélection MCTS (façon AlphaZero), réutilisable et rétrocompatible (`policy_prior=None` par défaut). Testé avec évaluation sans rollout (`reference/gen_bootstrap_dataset.py`, `bench_bootstrap.py`) : résultat **fortement négatif** (surapprentissage sur un signal circulaire, voir `reference/MODELS.md`, section "Pilote 3") -- le mécanisme PUCT lui-même reste sain et disponible pour une future tentative isolant mieux ses variables. |
