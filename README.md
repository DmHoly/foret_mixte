...
| `reference/gen_pairwise_dataset.py`, `train_pairwise_model.py` | Génère et entraîne le modèle de valeur contrastif linéaire (`pairwise_model.joblib`, celui utilisé par défaut). L'entraînement rapporte R²/MAE global **et** la précision de signe par tranche de `\|diff réel\|` (métrique qui prédit mieux la force réelle du modèle que R²/MAE seul, voir `reference/MODELS.md`) ; accepte aussi une repondération optionnelle vers les paires serrées (`tau`), testée et négative. |
| `reference/diagnose_pairwise_metrics.py` | Diagnostic autonome : précision de signe par tranche d'écart réel, sur k=1/k=5/bootstrap gen1 -- explique pourquoi R²/MAE global peut s'améliorer pendant que le résultat réel contre B empire, voir `reference/MODELS.md`. |
